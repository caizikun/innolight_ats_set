...
namespace ATS_Driver
{
    public class FLEX86100 : Scope
    {
            ...
            try
            {
                ...
                switch (IOType)
                {
                    case "GPIB":
                        ...
                        MY_Scope.IOConnect();
                        MY_Scope.WriteString("*IDN?");
                        EquipmentConnectflag = MY_Scope.ReadString().Contains("86100");
                        break;
                    default:
        ...
        public bool ConfigureCommonSetting(int syn = 0)
        {
             ...
             MY_Scope.WriteString(":display:persistence cgrade");//眼图显示为 多彩
        ...
        public double TestMask(string strChannel)
        {
            double CurrentMask = -100;
            double[] MaskArray= new double[4];
            for (int i = 0; i < 4; i++)

            {
                ...
                AcquisitionLimitTestSetup(acqLimitType, acqLimitNumber, 1);
                AcquisitionLimitTestSwitch(1, 1);
                AcquisitionControl(2);
                LoadMask(strChannel, opticalMaskName, 1);
                MaskTestMarginSetup(1, marginType, 0, marginHitType, marginHitRatio, marginHitCount, 1);
                MaskArray[i] = ReadMaskMargin();
     ...
     override   public bool MaskTestMarginSetup(byte marginOnOff, byte marginAutoManul, int manualMarginPercent, byte autoMarginType, double hitRatio, int hitCount, int syn = 0)
        ...
        public override bool DisplayEyeHeight()
        {
            try
            {
                return MY_Scope.WriteString(":MEASure:EYE:EyeHeight");
            }
            catch (Exception error)
            {
                logger.AdapterLogString(3, error.ToString());
        ...
        public override bool DisplayEyeWidth()
        {
            try
            {
                return MY_Scope.WriteString(":MEASure:EYE:EyeWidth");
            }
            catch (Exception error)
            {
                logger.AdapterLogString(3, error.ToString());
        ...
        private bool DisplayBitRate()
        {
            try
            {
                ...
                return MY_Scope.WriteString(":MEASure:EYE:BITRate");
            }
            catch (Exception error)
            {
                logger.AdapterLogString(3, error.ToString());
      ...
      override public bool DisplayER()
      {
        ...
        private double ReadBitRate()
        {
            ...
            try
            {
                MY_Scope.WriteString(":MEASure:EYE:BitRate?");
                double Str = Convert.ToDouble(MY_Scope.ReadString());
                biTRate = Str / (1E+9);
                //height = Convert.ToDouble(MY_Scope.ReadString(32)) * 1000.0;
