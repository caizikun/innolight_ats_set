...
#region Method
        ...
        private bool MultChannelTest()
        {
            ...
            for (int i = 0; i < pED.totalChannels;i++ )
            {
                ...
                dr["DmiRxPower"] = DmiRxPower;
                dr["DmiTxPower"] = DmiTxPower;
            ...
            else
            {
                ...
                if (isParametersComplete)
                {
                    for (byte i = 0; i < InformationList.Length; i++)
                    {
                        ...
                        if (algorithm.FindFileName(InformationList, "FLAGREADRXPOWRDMI", out index))
                        {
                            flagReadRxpowerDmi = Convert.ToBoolean(InformationList[index].DefaultValue);
                        }
                        if (algorithm.FindFileName(InformationList, "FLAGREADTXPOWRDMI", out index))
                        {
