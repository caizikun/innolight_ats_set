...
#region Method
        ...
        protected bool SerchCoefPoints(Attennuator tempAtt, ErrorDetector tempED, double startAttValue, double targetBer, double addStep, double sumStep, out ArrayList AttenPoints, out ArrayList BerPints)
        {
            ...
            do
            {
                tempAtt.AttnValue(startAttValue.ToString(), 1);
                //tempED.
                //tempAtten.SetAllChannnel_InputPower(startAttValue.ToString(), 0);
                // double[] ErrorArray = tempED.RapidErrorRate_4CH();
        ...
        protected double binarySearchTargetPoint(double StartInputPower ,double LLimit,double ULimit, double targetBerLL, double targetBerUL, Attennuator tempAtt, ErrorDetector tempED, out ArrayList serchAttPoints, out ArrayList serchBerPoints)
        {
            //double low = LLimit;
            //double high = ULimit;
            double currentCense = 0;
           ...
           // tempAtt
            ...
            while (CurrentInputPower <= ULimit && CurrentInputPower >= LLimit && count <= 20)
            {
        ...
        protected bool SerchTargetBer(Attennuator tempAtten, ErrorDetector tempED)
        {
            try
            {
                ...
                attPoint = binarySearchTargetPoint(testBerStruct.CsenStartingRxPwr,testBerStruct.SearchTargetBerRxpowerLL, testBerStruct.SearchTargetBerRxpowerUL, testBerStruct.SearchTargetBerLL, testBerStruct.SearchTargetBerUL, tempAtten, tempED, out serchAttPoints, out serchBerPoints);
