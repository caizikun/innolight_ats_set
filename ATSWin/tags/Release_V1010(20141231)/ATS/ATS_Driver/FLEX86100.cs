            ...
            try
            {
                ...
                else//未曾经设定过
                {
                    ...
                    for (byte i = 0; i < 4; i++)
                    {
                        ...
                        MY_Scope.WriteString(":system:mod? slot" + (i + 1).ToString());
                        returnmod = MY_Scope.ReadString();
                        if (returnmod.ToUpper().Contains("86107A"))
                        {
                            if (!(ptbch.Contains("86107A")))
                            {
                                ptbch.Add("86107A");
                                if (useptb)
                                {
                                ...
                                {
                                    PrecisionTimebaseSetup((byte)(i + 1), precisionTimebaseRefClk, 1, precisionTimebaseSynchMethod, syn);
                                }
