...
namespace Maintain
{
    public partial class MainForm : Form
    {
        ...
        {
            isReadEEPROM = (myLoginInfoStruct.myAccessCode & 0x01) == 0x01 ? true : false;
            isMaintainUser = (myLoginInfoStruct.myAccessCode & 0x200) == 0x200 ? true : false;
        ...
        bool getDSInfo()
        {
            try
            {
                ...
                mylastIDEEPROMID = Convert.ToInt32(myDataIO.GetLastInsertData("GlobalMSAEEPROMInitialize"));
        ...
        private void lstFileName_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                if (lstFileName.SelectedIndex != -1)
                {
                    ...
                    {
                        DataRow[] dr = GlobalDS.Tables["GlobalMSAEEPROMInitialize"].
        ...
        private void tsbtnDelete_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                int deleteItemCount = currPrmtrCountExisted(GlobalDS.Tables["GlobalMSAEEPROMInitialize"], "ItemName='" + myDeleteItemName + "' AND PID=" + EEPROMTabPID);
                ...
                if ((blnAddNew == true) && (deleteItemCount == 0))
                {
                    ...
                    if (drst == DialogResult.Yes)
                    {
                        ...
                        ResfeshList(GlobalDS.Tables["GlobalMSAEEPROMInitialize"], "PID=" + EEPROMTabPID, this.lstFileName, "ItemName");
                ...
                {
                    // 非新增状态下资料删除
                    string currID = getDTColumnInfo(GlobalDS.Tables["GlobalMSAEEPROMInitialize"], "ID", "ItemName='" + myDeleteItemName + "' AND PID=" + EEPROMTabPID);
                    ...
                    if (drst == DialogResult.Yes)
                    {
                        bool result = DeleteItemForDT(GlobalDS.Tables["GlobalMSAEEPROMInitialize"], "ID=" + currID + "and ItemName='" + myDeleteItemName + "'");
                        if (result)
                        {
                            MessageBox.Show("EEPROM FileName: " + myDeleteItemName + " has been removed!");
                            ResfeshList(GlobalDS.Tables["GlobalMSAEEPROMInitialize"], "PID=" + EEPROMTabPID, this.lstFileName, "ItemName");
        ...
        bool isCurrItemNameExisted(string str)
        {
            ...
            try
            {
                ...
                if (currPrmtrCountExisted(GlobalDS.Tables["GlobalMSAEEPROMInitialize"], "PID=" + EEPROMTabPID + " and ItemName ='" + lstFileName.Text + "' and " + str + " is not null") > 0)
                {
                    isExisted = true;
                    DataRow[] DR = GlobalDS.Tables["GlobalMSAEEPROMInitialize"].Select("PID=" + EEPROMTabPID + " and ItemName ='" + lstFileName.Text + "' and " + str + " is not null");
                    if (DR.Length == 1)
                    {
        ...
        bool isCurrItemNameExisted(string str, bool isCopy)
        {
            ...
            try
            {
                ...
                if (currPrmtrCountExisted(GlobalDS.Tables["GlobalMSAEEPROMInitialize"], "PID=" + GlobalCopyPID + " and ItemName ='" + GlobalCopyItemName + "' and " + str + " is not null") > 0)
                {
                    isExisted = true;
                    DataRow[] DR = GlobalDS.Tables["GlobalMSAEEPROMInitialize"].Select("PID=" + GlobalCopyPID + " and ItemName ='" + GlobalCopyItemName + "' and " + str + " is not null");
                    if (DR.Length == 1)
                    {
                        if (DR[0][str].ToString().Length == showRowsCount * 2)
                        {
        ...
        private void tsbtnSave_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (blnAddNew)
                {
                    if (currPrmtrCountExisted(GlobalDS.Tables["GlobalMSAEEPROMInitialize"], "PID=" + EEPROMTabPID + " and ItemName ='" + this.lstFileName.Text + "'") > 0)
                    {
                ...
                saveOK = EditEEPROMdata(GlobalDS.Tables["GlobalMSAEEPROMInitialize"]);
        ...
        private void tsbtnUpdate_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (GlobalDS.Tables["GlobalMSAEEPROMInitialize"].GetChanges() == null)
                {
                ...
                if (drst == DialogResult.Yes)
                {
                    ...
                    if (myDataIO.UpdateDataTable("select * from GlobalMSAEEPROMInitialize", GlobalDS.Tables["GlobalMSAEEPROMInitialize"]))
                    {
                        string time2 = "";
        ...
        string getTablesChanges() //更新后方可调用,否则datatable的RowState会被清除
        {
            ...
            try
            {
                ...
                SS += "**表[" + GlobalDS.Tables["GlobalMSAEEPROMInitialize"] + "]修改如下**\r\n";
                DataTable myDeletedDt = new DataTable();
                DataTable myAddDt = new DataTable();
                DataTable myChangeDt = new DataTable();
                myDeletedDt = GlobalDS.Tables["GlobalMSAEEPROMInitialize"].GetChanges(DataRowState.Deleted);
                myAddDt = GlobalDS.Tables["GlobalMSAEEPROMInitialize"].GetChanges(DataRowState.Added);
                myChangeDt = GlobalDS.Tables["GlobalMSAEEPROMInitialize"].GetChanges(DataRowState.Modified);
                ...
                if (myChangeDt != null)
                {
                    for (int j = 0; j < myChangeDt.Rows.Count; j++)
                    {
                        ...
                        for (int k = 0; k < dataRow.ItemArray.Length; k++)
                        {
                            ...
                            {
                                ss1 += GlobalDS.Tables["GlobalMSAEEPROMInitialize"].Columns[k].ColumnName + ":" + dataRow[k, DataRowVersion.Original].ToString() + ";";
                                ss2 += GlobalDS.Tables["GlobalMSAEEPROMInitialize"].Columns[k].ColumnName + ":" + dataRow[k, DataRowVersion.Current].ToString() + ";";
                            }
                        }
                        SS += ss1 + "\r\n";
                        SS += ss2 + "\r\n";
...
;
        ...
        string[] getEEPROMChangeLog(out string[] operationType) //将分为一个PItem一条维护记录!
        {
            ...
            try
            {
                //ConstGlobalListTables = new string[] { "GlobalMSAEEPROMInitialize", "GlobalAllEquipmentParamterList" };

                string toatalSS = "";
                string currOperationType = "";

                DataRow[] drsDelPItem = GlobalDS.Tables["GlobalMSAEEPROMInitialize"].Select("", "ID ASC", DataViewRowState.Deleted);

                #region tablesName is not deleted

                DataRow[] drsPItem = GlobalDS.Tables["GlobalMSAEEPROMInitialize"].Select("", "ID ASC");
                ...
                foreach (DataRow drTestPlan in drsPItem)
                {
                    ...
                    string ss = getDRChangeInfo(drTestPlan, "GlobalMSAEEPROMInitialize");
                    if (ss.Length > 0)
                    {
                        ss = "*[GlobalMSAEEPROMInitialize]*:  \r\n" + "<" + "PN = " + PN
                ...
                drsPItem = GlobalDS.Tables["GlobalMSAEEPROMInitialize"].Select("", "ID ASC", DataViewRowState.Deleted);
        ...
        private void tsmRnmCurrItem_Click(object sender, EventArgs e)
        {
            ...
            {
                string filterString = this.lstFileName.Text;
                DataRow[] myROWS = GlobalDS.Tables["GlobalMSAEEPROMInitialize"].Select("ItemName='" + filterString + "' AND PID=" + EEPROMTabPID);
                if (myROWS.Length == 1)
                {
                    myROWS[0].BeginEdit();
