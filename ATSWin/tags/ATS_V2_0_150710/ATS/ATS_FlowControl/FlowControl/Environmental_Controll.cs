...
namespace ATS
{
    public class EnvironmentalControll
    {
        ...
        public bool ConfigEnvironmental(double Temp, double Voltage, byte Channel,Double TempOffset,int TempWaitTime)
        {
            CurrentTemp = Temp;
            ...
            if (XstreamName != null && XstreamName != "")
            {
                if (Temp != LastTemp)
                {
                   ...
                   // Thread.Sleep(SleepTime*1000);
                    ...
                    {
                       // prosess = Convert.ToInt16(k * 100 / SleepTime);
                        TempProcess = Convert.ToInt16(k * 100 / SleepTime);
                        Thread.Sleep(SleepTime * 1000);
                    }
                }
