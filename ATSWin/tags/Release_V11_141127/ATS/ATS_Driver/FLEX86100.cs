...
namespace ATS_Driver
{
    public class FLEX86100 : Scope
    {
        ...
        public override bool ChangeChannel(string channel, int syn = 0)
        {
            ...
            for (int i = 0; i < 4; i++) 
            {
                ChannelDisplaySwitch(strElecChannel[i], 0);
                ChannelDisplaySwitch(strOpticalChannel[i], 0);
            }
                return true;
        }
        ...
        public override bool Configure(int syn = 0)
        {
            try
            {
                ...
                else//未曾经设定过
                {
                    ...
                    WavelengthSelect(strOpticalChannel[0], FlexDcaWavelength, syn);
                    for (byte i = 0; i < 4; i++)
                    {
                        WavelengthSelect(strElecChannel[i], FlexDcaWavelength, syn);
                        FileterSelect(strOpticalChannel[i], FlexDcaDataRate);
                        FileterSelect(strElecChannel[i], FlexDcaDataRate);
                        FileterSwitch(strOpticalChannel[i], FilterSwitch, syn);
                        FileterSwitch(strElecChannel[i], FilterSwitch, syn);
                    }
                    for (byte i = 1; i < 5; i++)
                    {
                    ...
                    for (byte i = 0; i < 4; i++)
                    {
                        ...
                        SetAttenuation(strElecChannel[i], FlexDcaAtt, opticalAttSwitch, syn);
                        SetERCorrectFactor(strOpticalChannel[i], erFactor, erFactorSwitch, syn);
                        SetERCorrectFactor(strElecChannel[i], erFactor, erFactorSwitch, syn);
                    }
                    //添加判断，使用PTB or86107A
                    ...
                    for (byte i = 0; i < 4; i++)
                    {
                        ...
                        ChannelDisplaySwitch(strElecChannel[i], 0, syn);
                        EyeScaleOffset(strOpticalChannel[i], FlexScale, FlexOffset, syn);
                        EyeScaleOffset(strElecChannel[i], FlexScale, FlexOffset, syn);
                        LoadMask(strOpticalChannel[i], opticalMaskName, syn);
                        LoadMask(strElecChannel[i], elecMaskName, syn);
                    }
                    MaskONOFF(false, syn);
        ...
        public override double[] OpticalEyeTest(int syn=0)
        {
            ...
            try
            {
              ...
              //  SetAttenuation(strChannel, FlexDcaAtt, opticalAttSwitch);//channel change have setted
                ...
                MaskTestMarginSetup(1, marginType, 0, marginHitType, marginHitRatio, marginHitCount, syn);
                SaveEyeDiagram(pglobalParameters.StrPathOEyeDiagram, 0, 0);
        ...
        public override double[] ElecEyeTest(int syn=0)
        {
            ...
            try
            {
                ...
                MaskTestMarginSetup(1, marginType, 0, marginHitType, marginHitRatio, marginHitCount, syn);
                SaveEyeDiagram(pglobalParameters.StrPathEEyeDiagram, 0, 0);
        ...
        override public bool OpenOpticalChannel(bool Switch, int syn = 0)//true optical  false elec
        {
            ...
            {
                return ChannelDisplaySwitch(strElecChannel[Convert.ToByte(CurrentChannel) - 1], 1, syn);
            }
        }
