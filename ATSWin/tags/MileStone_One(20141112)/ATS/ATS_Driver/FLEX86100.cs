...
namespace ATS_Driver
{
    public class FLEX86100 : Scope
    {
        ...
        public override bool ChangeChannel(string channel, int syn = 0)
        {
            ...
            return SetAttenuation(dcacurrentchannel, temp, 1,syn);
        ...
        public override bool Configure(int syn = 0)
        {
            try
            {
                ...
                else//未曾经设定过
                {
                    ...
                    RapidEyeSetup(FlexDcaDataRate, rapidEyeSwitch, syn);
                    TriggerSetup(triggerSource, FlexTriggerBwlimit, syn);
                    WavelengthSelect(strOpticalChannel, FlexDcaWavelength, syn);
                    WavelengthSelect(strElecChannel, FlexDcaWavelength, syn);
                    FileterSelect(strOpticalChannel, FlexDcaDataRate);
                    FileterSelect(strElecChannel, FlexDcaDataRate);
                    FileterSwitch(strOpticalChannel, FilterSwitch, syn);
                    FileterSwitch(strElecChannel, FilterSwitch, syn);
                    ...
                    SetAttenuation(strOpticalChannel, FlexDcaAtt, opticalAttSwitch, syn);
                    SetAttenuation(strElecChannel, FlexDcaAtt, opticalAttSwitch, syn);
                    SetERCorrectFactor(strOpticalChannel, erFactor, erFactorSwitch, syn);
                    SetERCorrectFactor(strElecChannel, erFactor, erFactorSwitch, syn);
                    //添加判断，使用PTB or86107A
                    MY_Scope.WriteString(":SYSTem:OPT?");
                    string returnptb = MY_Scope.ReadString();
                    string temp = returnptb.ToUpper().Replace(" ", "");
                    bool useptb = temp.Contains("PTB");
                    if (useptb)
                    {
                        for (ptbslot = 1; ptbslot < 5; ptbslot++)
                        {
                            MY_Scope.WriteString(":system:mod? slot" + ptbslot);
                            string returnmod = MY_Scope.ReadString();
                            if (returnmod == "86107A")
                            {
                                MessageBox.Show("PTB 86107A冲突，请取出86107A模组再继续！ ");
                               break;
                            }
                        }     
                    ...
                    else
                    {
                        for (ptbslot = 1; ptbslot < 5; ptbslot++)
                        {
                            ...
                            if (returnmod == "86107A")
                            {
                                PrecisionTimebaseSetup(ptbslot, precisionTimebaseRefClk, 1, precisionTimebaseSynchMethod, syn);
                                break;
                            }
                        }
                    }
                    MessureThresholdSetup(Threshold, reference, syn);
                    ChannelDisplaySwitch(strOpticalChannel, 1, syn);
                    ChannelDisplaySwitch(strElecChannel, 0, syn);
                    EyeScaleOffset(strOpticalChannel, FlexScale, FlexOffset, syn);
                    EyeScaleOffset(strElecChannel, FlexScale, FlexOffset, syn);
                    LoadMask(strOpticalChannel, opticalMaskName, syn);
                    LoadMask(strElecChannel, elecMaskName, syn);
                    MaskONOFF(false, syn);
        ...
        /// </summary>
        /// <returns></returns>
        public override double[] OpticalEyeTest(int syn=0)
        {
        ...
        /// </summary>
        /// <returns></returns>
        public override double[] ElecEyeTest(int syn=0)
        {
        ...
        private bool SetAttenuation(string channel, double attenuation, byte attSwitch, int syn = 0)
        {
            ...
            try
            {
                if (attSwitch == 1)
                {
                    ...
                    else
                    {
                        ...
                        if ((flag1 == true) && (flag2 == true))
                        {
                            ...
                            {
                                logger.AdapterLogString(3, "set attenuation wrong");
                ...
                else
                {
                    if (syn == 0)
                    {
                        logger.AdapterLogString(0, "ATTenuator STATe IS off");
                    ...
                    else
                    {
                        ...
                        if (flag1 == true)
                        {
                            ...
                            {
                                logger.AdapterLogString(3, "set ATTenuator STATe wrong");
                ...
                if (erFactorSwitch == 1)
                {
                    ...
                    else
                    {
                        ...
                        if ((flag1 == true) && (flag2 == true))
                        {
                            ...
                            {
                                logger.AdapterLogString(3, "set erFactorPercent wrong");
            ...
            try
            {
                ...
                else
                {
                    ...
                    else
                    {
                        for (int i = 0; i < 3; i++)
                        {
                            flag1 = MY_Scope.WriteString(":ACQ:REYE " + "OFF");
                            if (flag1 == true)
                                break;
                        }
                        if (flag1 == true)
            ...
            override public bool OpenOpticalChannel(bool Switch,int syn=0)//true optical  false elec
        {
            if (Switch)
            {
              ...
              return ChannelDisplaySwitch(dcacurrentchannel, 1,syn);
            }
            else
            {
        ...
        public override bool SetScaleOffset(int syn=0)
        {
            try
            {
                string strOpticalChannel = AssembleChannelString(opticalSlot, FlexOptChannel);
                ...
                EyeScaleOffset(strOpticalChannel, scaletemp, offsettemp,syn);
      ...
      override public bool DisplayER()
      {
        ...
        private bool SaveEyeDiagram(string savePath, byte picColor, byte bgColor)
        {
            ...
            try
            {
                ...
                diagramName = pglobalParameters.CurrentSN + "-" + pglobalParameters.CurrentTemp + "-" + pglobalParameters.CurrentVcc + "-" + pglobalParameters.CurrentChannel + "-" + Str_Year + "_" + Str_Month + "_" + Str_Day + "_" + Str_Hour + "_" + Str_minute + ".bmp";
