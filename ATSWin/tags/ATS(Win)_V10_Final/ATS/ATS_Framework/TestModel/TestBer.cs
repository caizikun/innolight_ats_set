...
#region Method
        ...
        protected override bool StartTest()
        {
            ...
            if (selectedEquipList["ATTEN"] != null && selectedEquipList["ERRORDETE"] != null && selectedEquipList["DUT"] != null && selectedEquipList["POWERSUPPLY"] != null)
            {
                ...
                for(int i=0;i<Convert.ToInt16(tempAtten.TotalChannel);i++)
                {
                    tempAtten.ChangeChannel((i + 1).ToString());
                    tempAtten.AttnValue("5");
                }
                tempAtten.ChangeChannel(GlobalParameters.CurrentChannel.ToString());
                ...
                if (isAutoAlaign)
                {
                    if (testBerStruct.IsBerQuickTest==true)
                    {
                        ...
                        else if (testBerStruct.CsenTargetBER == 5E-5)
                        {
                             logoStr += logger.AdapterLogString(1, "AttPoint=" + testBerStruct.CsenStartingRxPwr.ToString() + " ErrorRate="+sensitivity);
                            ...
                            {
                                sensitivityPoint = sensitivity;
                                logoStr += logger.AdapterLogString(4, "AttPoint=" + testBerStruct.CsenStartingRxPwr.ToString() + "CSENCE> 5E-5 ");
                                AnalysisOutputParameters(outputParameters);
                                logger.FlushLogBuffer();
                                return false;
                            }
                    ...
                    else
                    {
                        ...
                        if (testBerStruct.CsenTargetBER == 1E-3 || testBerStruct.CsenTargetBER == 5E-5)
                        {
                            ...
                            //{
                            //    logoStr += logger.AdapterLogString(1, "attPointsBeforDelErrPoints[ " + i.ToString() + "]" + double.Parse(attPoints[i].ToString()) + "  " + "berPointsBeforDelErrPoints[ " + i.ToString() + "]" + double.Parse(berPoints[i].ToString()));
                            //}
                        ...
                        else if (testBerStruct.CsenTargetBER == 1E-12 || testBerStruct.CsenTargetBER == 1E-15)
                        {
        ...
        protected bool SerchCoefPointsTarget1E3(Attennuator tempAtt, ErrorDetector tempED, double startAttValue, double targetBer, double addStep, double sumStep, out ArrayList AttenPoints, out ArrayList BerPints)
        {
            ...
            } while (i < 8 && (currentBer < (targetBer)));
