        ...
        {        
            ...
            else//  InformationList is not null
            {
                ...
                for (byte i = 0; i < InformationList.Length; i++)
                {
                    ...
                    if (algorithm.FindFileName(InformationList, "LOSA(OMA)", out index))
                    {
                        losAOma = algorithm.CalculateOMA(losA, Convert.ToDouble(OSERValueArray[GlobalParameters.CurrentChannel - 1]));
                        InformationList[index].DefaultValue = Math.Round(losAOma, 4).ToString().Trim();
                    }
                    if (algorithm.FindFileName(InformationList, "LOSD(OMA)", out index))
                    {
                        losDOma = algorithm.CalculateOMA(losD, Convert.ToDouble(OSERValueArray[GlobalParameters.CurrentChannel - 1]));
                        InformationList[index].DefaultValue = Math.Round(losDOma, 4).ToString().Trim();
                    }
                }
                return true;
            }
