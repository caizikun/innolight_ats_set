    ...
    public class MAP200Atten : Attennuator
    {
        ...
        public override bool Initialize(TestModeEquipmentParameters[] MAP200AttenStruct)
        {
            try
            {
                ...
                {
                    Addr = MAP200AttenStruct[i].DefaultValue;
                }
                else
                {
        ...
        public override bool AttnValue(string InputPow, int syn = 0) 
        {
            bool flag1 = false;
            double dAttValue = 0;
            try
            {
                // 仪器中的Offset将不在添加.在计算中使用.
                dAttValue = currentOffset-Convert.ToDouble(InputPow) ;
                ...
                //AttTemp = Convert.ToDouble(Value) - CurrentOffset;
                //Value = Math.Abs((dAttValue.ToDouble(Value) - CurrentOffset)).ToString();// 仪器中的Offset将不在添加.在计算中使用.
