    ...
    public partial class MainForm : Form
    {
        ...
        {           
            ...
            {  
                ...
                cboOpItem.Items.Clear();
                cboOpItem.Items.Add("AllOpTypeItems");
                DataRow [] drs = myResultDt.DefaultView.ToTable(true, "OpType").Select("");
                foreach (DataRow currDr in drs)
                {
                    cboOpItem.Items.Add(currDr["OpType"].ToString());
                }
                cboOpItem.SelectedIndex = 0;
        ...
        private void btnQuery_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                //150521
                if (cboOpItem.SelectedIndex != 0)
                {
                    if (selectConditions.Length > 0)
                    {
                        selectConditions += " and opType LIKE '%" + this.cboOpItem.Text + "%' ";                       
                    }
                    else
                    {
                        selectConditions += " opType LIKE '%" + this.cboOpItem.Text + "%' ";
                    }
                }
        ...
        private void lstOPType_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    string currLstItemStr = lstOPType.SelectedItem.ToString().ToUpper();
                    string searchString = "".PadRight(80, '#') + "\n" + lstOPType.SelectedItem.ToString().ToUpper();
                    string totalStr = rtxt.Text.ToUpper();
                    ...
                    {
                        count++;
                        index = rtxt.Text.ToUpper().IndexOf(searchString, startIndex);
                        if (index != -1)
                        {                            
        ...
        private void cboOpItem_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (cboOpItem.SelectedIndex != -1)
            {
                myTip = new ToolTip();
                myTip.SetToolTip(this.cboOpItem, this.cboOpItem.Text);
