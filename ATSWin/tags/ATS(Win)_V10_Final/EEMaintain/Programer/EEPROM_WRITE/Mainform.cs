...
namespace Maintain
{
    public partial class MainForm : Form
    {
        ...
        ValidationRule pValidationRule = new ValidationRule();      //验证权限变更
        bool isProgramEEPROMUser = false;
        ...
        private void Mainform_Load(object sender, EventArgs e)
        {
            ...
            isProgramEEPROMUser = pValidationRule.CheckLoginAccess(LoginAppName.EEPROMProgram, CheckAccess.ProgramEEPROM, myLoginInfoStruct.myAccessCode); 

            blnISDBSQLserver = myLoginInfoStruct.blnISDBSQLserver;

            //if (isEEPROMWriter)
            if (isProgramEEPROMUser)
            {
            }
            else
            {
        ...
        private void cboPNItem_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
               if (cboPNItem.Text.ToString().Trim().ToUpper() != myLastItem)
               {
                   ...
                   {                      
                       setFormState(true);
                       DataRow[] dr = GlobalDS.Tables["TopoMSAEEPROMSet"].Select("PID=" + EEPROMTabPID + " and ItemName ='" + cboPNItem.Text + "'");
