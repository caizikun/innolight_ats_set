       ...
       protected override bool ConfigureEquipment(EquipmentList selectedEquipmentList)
        ...
        protected override bool StartTest()
        {
            ...
            if (selectedEquipList["DUT"] != null && selectedEquipList["ATTEN"] != null && selectedEquipList["POWERSUPPLY"] != null)
            {
                ...
                {
                    tempAtten.AttnValue(testRxPowerDmiStruct.ArrayListRxInputPower[i].ToString(),0);  
        ...
        protected override bool AnalysisOutputParameters(TestModeEquipmentParameters[] InformationList)
        {
            ...
            else//  InformationList is not null
            {
                ...
                for (byte i = 0; i < InformationList.Length; i++)
                {
                    ...
                    if (algorithm.FindFileName(InformationList, "DMIRXNOPTICAL(DBM)", out index))
                    {
                        RxNopticalPoint = algorithm.ISNaNorIfinity(RxNopticalPoint);
                        InformationList[index].DefaultValue = Math.Round(RxNopticalPoint, 4).ToString().Trim();
