    ...
    public class EEPROM : EquipmentBase
    {
        ...
        public bool u8tobyte(int deviceIndex, int deviceAddress, int regAddress, UInt16 coef, int phycialAdress = 0, int mdiomode = 0)
        {
            ...
            else
            {
                ...
                for (int i = 0; i < 4; i++)
                {
                    byte[] rcoef = new byte[1];
                    rcoef = USBIO.ReadReg(deviceIndex, deviceAddress, regAddress, IOPort.SoftHard.HARDWARE_SEQUENT, 1);
                    if ((buff[0] != rcoef[0]) )
                    {
                        USBIO.WrtieReg(deviceIndex, deviceAddress, regAddress, IOPort.SoftHard.HARDWARE_SEQUENT, buff);
                        System.Threading.Thread.Sleep(200);
        ...
        public byte[] ReadWriteDriverSFP_Old(int deviceIndex, int deviceAddress, int StartAddress, int regAddress, byte operation, byte chipset, byte[] buffer)
        {
            //database 0: LDD 1: AMP 2: DAC 3: CDR
        ...
        public byte[] ReadWriteDriverSFP28(int deviceIndex, int deviceAddress, int StartAddress, int regAddress, byte operation, byte chipset, byte[] buffer)
        {
            //database 0: LDD 1: AMP 2: DAC 3: CDR
        ...
        public byte[] ReadWriteDriverSFP(int deviceIndex, int deviceAddress, int StartAddress, int regAddress, byte operation, byte chipset, byte[] buffer)
        {
            ...
            try
            {
              ...
              //  USBIO.WrtieReg(deviceIndex, deviceAddress, StartAddress + 8, IOPort.SoftHard.HARDWARE_SEQUENT, tempData1);
                ...
                {
                    System.Threading.Thread.Sleep(50);
                    byte[] temp = USBIO.ReadReg(deviceIndex, deviceAddress, StartAddress + 3, IOPort.SoftHard.HARDWARE_SEQUENT, 1);
                    if (temp[0] == 0)
                    {
