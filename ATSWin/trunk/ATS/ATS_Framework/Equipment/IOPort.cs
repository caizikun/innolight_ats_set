...
namespace ATS_Framework
{
    ...
    public class IOPort
    {
        ...
        public bool IOConnect()
        {
            try
            {
                switch (Str_IO_interface.ToUpper())
                {
        ...
        public void CFPCtrlNorPinBase(int deviceIndex, byte cmd, byte rw, byte DDR, byte setData, out byte getData)
        {
            OpenDevice(deviceIndex);
            CH375SetTimeout((byte)deviceIndex, 2000, 2000);
            byte[] buffer = new byte[6];
            buffer[0] = 0;
            buffer[1] = 0;
            buffer[2] = cmd;
       ...
       // logManager log=new logManager();
        public bool WriteString(string Str_Write)
        {
            try
            {
                switch (Str_IO_interface.ToUpper().Trim())
                {
                    case "GPIB":
...
#region  25G-BERT
...
#endregion
        public string GPIBQuery(string QueryStr = "*OPC?")
        {
            string Str = "";
            try
            {
                ...
                {
                    case "GPIB":
                        WriteString(QueryStr);
                        Thread.Sleep(50);
                        Str = myDmm.ReadString().Replace("\n", "");
                        break;
                    case "USB":
                        break;
                    case "RJ45":// 网口
        ...
        public bool QueryOpc(string QueryStr = "*OPC?")
        {
            bool flag = false;
            string Str = "";
            try
        ...
        public bool WriteString(string Str_Write, int syn = 0)
        {
            bool flag = false;
            try
            {
