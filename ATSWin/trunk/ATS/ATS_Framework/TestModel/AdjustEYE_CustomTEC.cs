...
namespace ATS_Framework
{
    ...
    public class AdjustEye_CustomTEC : TestModelBase
    { 
#region Attribute
...
#region Method
        ...
        public AdjustEye_CustomTEC(DUT inPuDut, logManager logmanager)
        {
            SpecNameArray.Clear();
            logger = logmanager;           
            logoStr = null;
            ...
            inPutParametersNameArray.Add("CustomTemp");
            //DCCOUPLEADJUSTMETHOD
            inPutParametersNameArray.Add("DCCOUPLEADJUSTMETHOD");
            //...
            ...
            else
            {
                ...
                if (isParametersComplete)
                {
                    //for (byte i = 0; i < InformationList.Length; i++)
                    {
                        ...
                        if (algorithm.FindFileName(InformationList, "CustomTemp", out index))
                        {
                            double temp = Convert.ToDouble(InformationList[index].DefaultValue);
                            if (double.IsInfinity(temp) || double.IsNaN(temp))
                            {
            ...
            try
            {
                ...
                if (!tempratureADCArray.ContainsKey(GlobalParameters.CurrentTemp.ToString().Trim().ToUpper()))
                {
                  ...
                  //  dut.ReadTempADC(out tempratureADC, 1);
                    ...
                    if (dCustomTemp != -100)//表示 需要计算 TempADC
                    {
                        ...
                        if (Coefb == 0)
                        {
                            tempratureADC = Convert.ToInt16(Coefc);
                        }
                        else
                        {
                            tempratureADC = Convert.ToInt16((dCustomTemp*256 - Coefc) / Coefb);
                        }
                        tempratureADCArrayList.Add(tempratureADC);//tempratureADCArray
                        
                    }
                    else
                    {
                        UInt16 UtempratureADC;
                        dut.ReadTempADC(out UtempratureADC, 1);
                        tempratureADCArrayList.Add(UtempratureADC);
