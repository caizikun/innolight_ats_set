...
namespace ATS_Driver
{
    public class SFP28DUT : DUT
    {
      ...
      #region FullFuntion
        ...
        private bool CDR_Enable()
        {
            ...
            try
            {
                if (IsCDROn)
                {
                    ...
                    buff = USBIO.ReadReg(deviceIndex, 0XA2, 118, IOPort.SoftHard.HARDWARE_SEQUENT, 1);
                    buff[0] = (byte)(buff[0] | 0x08);//确保Bit 3=1
                    TempBuff = buff[0];
                    for (int i = 0; i < 3; i++)
                    {

                        USBIO.WrtieReg(deviceIndex, 0XA2, 118, IOPort.SoftHard.HARDWARE_SEQUENT, buff);
                        System.Threading.Thread.Sleep(200);
                        buff = USBIO.ReadReg(deviceIndex, 0XA2, 118, IOPort.SoftHard.HARDWARE_SEQUENT, 1);
                        if (buff[0] == TempBuff)
                        {
        ...
        {
            return sfp.ReadPn(deviceIndex, 0xA0, 40).Trim();
        }
        ...
        public byte[] WriteDriverSFP28(int deviceIndex, int deviceAddress, int StartAddress, int regAddress, byte chipset, byte[] dataToWrite)
        {
            ...
            return sfp.ReadWriteDriverSFP28(deviceIndex, deviceAddress, StartAddress, regAddress, 0x02, chipset, dataToWrite);
        }
        ...
        {
            return sfp.ReadWriteDriverSFP28(deviceIndex, deviceAddress, StartAddress, regAddress, 0x01, chipset, new byte[2]);
        }
        ...
        public byte[] StoreDriverSFP28(int deviceIndex, int deviceAddress, int StartAddress, int regAddress, byte chipset, byte[] dataToWrite)
        {
            ...
            return sfp.ReadWriteDriverSFP28(deviceIndex, deviceAddress, StartAddress, regAddress, 0x06, chipset, dataToWrite);
        }
                ...
                for (int i = 0; i < DriverInitStruct.Length; i++)
                {
                    ...
                    else
                    {
                     ...
                     byte[] WriteBiasDacByteArray=WriteBiasDacByteArray = algorithm.ObjectTOByteArray(DriverInitStruct[i].ItemValue, 2, DriverInitStruct[i].Endianness);
                        ...
                        for (k = 0; k < 3; k++)
                        {
                            WriteDriverSFP28(deviceIndex, 0xA2, startaddr, DriverInitStruct[i].RegisterAddress, chipset, WriteBiasDacByteArray);
 
                            StoreDriverSFP28(deviceIndex, 0xA2, startaddr, DriverInitStruct[i].RegisterAddress, chipset, WriteBiasDacByteArray);
                            // Thread.Sleep(200);  
                          //  temp = new byte[DriverInitStruct[i].Length];
                            temp = ReadDriverSFP28(deviceIndex, 0xA2, startaddr, DriverInitStruct[i].RegisterAddress,  chipset);
            ...
            else
            {
                for (int i = 0; i < EEpromInitStruct.Length; i++)
                {
                    ...
                    for (k = 0; k < 3; k++)
                    {
                       ...
                       // temp = new byte[1];
                        temp = ReadReg(deviceIndex, EEpromInitStruct[i].SlaveAddress, EEpromInitStruct[i].StartAddress, EEpromInitStruct[i].Length);
                        if (BitConverter.ToString(temp) == BitConverter.ToString(WriteBiasDacByteArray))
                            break;
                    }
                    if (k >= 3)
...
#region  RXAPD
        ...
        public override bool SetAPDdaccoefa(string apddaccoefa)
        {
            ...
            if (FindFiledNameChannel(out i, "DmiAPDCoefA"))
            {
                ...
                try
                {
                    flag = sfp.SetCoef(deviceIndex, 0xA2, DutStruct[i].StartAddress, apddaccoefa, DutStruct[i].Format);
                    logger.AdapterLogString(1, "SET DmiAPDCoefA To" + apddaccoefa);
                    return flag;
                }
                catch (Exception error)
                {
            ...
            {
                logger.AdapterLogString(4, "there is no DmiAPDCoefA");
        ...
        public override bool SetAPDdaccoefb(string apddaccoefb)
        {
            ...
            if (FindFiledNameChannel(out i, "DmiAPDCoefb"))
            {
                ...
                try
                {
                    flag = sfp.SetCoef(deviceIndex, 0xA2, DutStruct[i].StartAddress, apddaccoefb, DutStruct[i].Format);
                    logger.AdapterLogString(1, "SET DmiAPDCoefA To" + apddaccoefb);
                    return flag;
                }
                catch (Exception error)
                {
            ...
            {
                logger.AdapterLogString(4, "there is no apddaccoefb");
        ...
        public override bool SetAPDdaccoefc(string apddaccoefc)
        {
            ...
            if (FindFiledNameChannel(out i, "DmiAPDCoefC"))
            {
                ...
                try
                {
                    flag = sfp.SetCoef(deviceIndex, 0xA2, DutStruct[i].StartAddress, apddaccoefc, DutStruct[i].Format);
                    logger.AdapterLogString(1, "SET DmiAPDCoefC To" + apddaccoefc);
                    return flag;
                }
                catch (Exception error)
                {
            ...
            {
                logger.AdapterLogString(4, "there is no DmiAPDCoefC");
        ...
        public override bool SetRXPadcThreshold(UInt16 threshold)
        {
            ...
            if (FindFiledNameChannel(out i, "RXADCTHRESHOLD"))
            {
                ...
                {
                      logger.AdapterLogString(3, " RXADCTHRESHOLD TO Large");
                      return false;
                }
                try
                {
                 ...
                 if (FindFiledNameChannelDAC(out i, StrItem))
                 {
                     ...
                     for (int J = 0; J < 3; J++)
                     {
                         ...
                         if (operate == 0)//写
                         {
                             ...
                             WriteDriverSFP28(deviceIndex, 0xA2, startaddr, DriverStruct[i].RegisterAddress, chipset, WriteDacByteArray);
                         }
                         else//存
                         {

                             StoreDriverSFP28(deviceIndex, 0xA2, startaddr, DriverStruct[i].RegisterAddress, chipset, WriteDacByteArray);

                         }

                         byte[] ReadDacByteArray = ReadDriverSFP28(deviceIndex, 0xA2, startaddr, DriverStruct[i].RegisterAddress, chipset);
         ...
         private bool ReadDAC(string StrItem, out int ReadDAC)
         {
             ...
             try
             {
                 ...
                 if (FindFiledNameChannelDAC(out i, StrItem))
                 {
                     ...
                     byte[] DacArray = ReadDriverSFP28(deviceIndex, 0xA2, startaddr, DriverStruct[i].RegisterAddress, chipset);
        ...
        #region  SFP28-LR-IT_CurveError

        public override bool SetBiasCurrent(string TempPoint, float Current)
        {
            string StrItemName;
            StrItemName = TempPoint.ToUpper() + "T_Ibias ";


            float k = Current * 500;

            return SetCoef(StrItemName.ToUpper(), k);
        }


        public override bool SetTcase(string TempPoint, float Temp)
        {
            string StrItemName;
            StrItemName = TempPoint.ToUpper() + "tCaseTemp";

            byte TempValue = (byte)Temp;
            float k = (float)TempValue;
            return SetCoef(StrItemName.ToUpper(), k);
        }

        public override bool SetTE(string TempPoint, float TE)
        {
            string StrItemName;
            StrItemName = TempPoint + "tTECoef";
            return SetCoef(StrItemName, TE);
         
        }

        public override bool SetSE(string TempPoint, float SE)
        {
            string StrItemName;

           
            StrItemName = TempPoint + "tSeCoef";
            return SetCoef(StrItemName, SE);

        }

        //public override bool SetBiasDAC(string TempPoint, Int16 DAC)
        //{
        //    string StrItemName;       
        //    StrItemName = TempPoint + "TBiasDACPoint";
        //    return SetCoef(StrItemName, DAC);
        //}

        public override bool SetModDAC(string TempPoint, short DAC)
        {

            string StrItemName;
            StrItemName = TempPoint + "tModDACPoint";
            return SetCoef(StrItemName, DAC);
        }

        public override bool SetRatioOMA(string TempPoint, float Ratio_OMA)
        {
            

            string StrItemName;
            StrItemName = TempPoint + "TRatioOmACoef";
            return SetCoef(StrItemName, Ratio_OMA);
        }

        public override bool SetRatioPf(string TempPoint, float Ratio_Pf)
        {
         
            string StrItemName;
            StrItemName = TempPoint + "tRatioPfCoef";
            return SetCoef(StrItemName, Ratio_Pf);
        }

        public override bool SetIthNorm(string TempPoint, float IthNorm)
        {
            string StrItemName;

            StrItemName = TempPoint + "tIthNormCoef";
           

            return SetCoef(StrItemName, IthNorm);
        }

        public override bool SetSeDiffTemp(string TempPoint, float IthNorm)
        {
            string StrItemName;
            StrItemName = TempPoint + "tSeDiffTemp";
            return SetCoef(StrItemName, IthNorm);
        }

         private  bool SetCoef(string ItemName,float SetValue )
        {
            ...
            if (FindFiledNameChannel(out i, ItemName.ToUpper().Trim()))
            {
                Engmod(DutStruct[i].EngPage);
                try
                {
                    flag = sfp.SetCoef(deviceIndex, 0xA2, DutStruct[i].StartAddress, SetValue.ToString(), DutStruct[i].Format);
                    logger.AdapterLogString(1, ItemName + "=" + SetValue);
                    return flag;
                }
                catch (Exception error)
                {
                    logger.AdapterLogString(3, "Set"+ ItemName+" Error," + error.ToString());
                    return flag;
                }
            }
            else
            {
                logger.AdapterLogString(3, "Set" + ItemName + " Error!");
                return flag;
            }
        }
