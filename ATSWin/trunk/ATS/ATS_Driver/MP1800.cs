...
namespace ATS_Driver
{
    public class MP1800PPG : PPG
    {
        ...
        public override bool Initialize(TestModeEquipmentParameters[] MP1800PPGStruct)
        {
            ...
            {
                logger.AdapterLogString(4, "there is no OperationBitrate");
                return false;
            }
        ...
        private bool ConfigureOperationBitrate(byte OperationRate, int syn = 0)//PPG比特率，单位为Gbps.
        {
            ...
            try
            {
                ...
                logger.AdapterLogString(0, "MP1800ppg OperationRate is" + StrRate);
           
                 MyIO.WriteString(":SYSTem:OUTPut:BITRate " + StrRate);

                if (syn == 0)
                {
                    logger.AdapterLogString(0, "MP1800ppg OperationRate is" + StrRate);
                   // string STR = ":SYSTem:OUTPut:BITRate " + StrRate;
                    return MyIO.WriteString(":SYSTem:OUTPut:BITRate " + StrRate);
                }
                else
                {
                    for (int i = 0; i < 3; i++)
                    {
                        flag1 = MyIO.WriteString(":SYSTem:OUTPut:BITRate?");
                        if (flag1 == true)
                            break;
                    }
                    if (flag1 == true)
                    {
                        ...
                        if (k < 3)
                        {
                            logger.AdapterLogString(0, "MP1800ppg OperationRate is" + dataRate);
                            flag = true;
                        }
                        else
                        {
                            logger.AdapterLogString(3, "set MP1800ppg OperationRate wrong");
