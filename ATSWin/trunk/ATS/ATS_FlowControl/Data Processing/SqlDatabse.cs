...
namespace ATS
{
...

    {
       // private SqlConnection conn = null;
        ...
        /// 执行多条SQL语句，实现数据库事务。
        /// </summary>
        /// <param name="sqlCmdLst">字符语句列表</param>
        /// <returns>返回值true成功;否则失败</returns>
        public override bool SqlExeSqlTran(List<string> sqlCmdLst)
        {
            OpenDatabase(true);
            ...
            int Exint = -1;
            string strsql = "";
            for (int n = 0; n < sqlCmdLst.Count; n++)
            {
                if (sqlCmdLst[n].ToString().Trim().Length > 0)
                {
                    strsql += sqlCmdLst[n].ToString();
                }
            }
            if (strsql.Trim().Length > 0)
            {
                ...
                {
                    //LogHelper.WriteLogs(er.ToString());
                    Exint = 0;
                    throw er;
                }
            }
