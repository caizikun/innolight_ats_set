...
namespace ATS
{
     ...
     */
        ...
        private ArrayList LightSourceVecpArray = new ArrayList();
       ...
       //----------------------------
        ...
        private bool FlagGetSqlParameterOK = false;
...
#region  委托声明
        ...
        private bool ReadXmlInf()
        {
           ...
           //------------------AttOffset
            ...
            //------------------LightSourceVECP
            StrValue = MyXml.LightSourceVecp;
            Array1 = StrValue.Split(',');
            LightSourceVecpArray.Clear();

            for (int i = 0; i < Array1.Length; i++)
            {
                LightSourceVecpArray.Add(Array1[i]);
            }
            textBoxLsVecp1.Text = Array1[0];
            textBoxLsVecp2.Text = Array1[1];
            textBoxLsVecp3.Text = Array1[2];
            textBoxLsVecp4.Text = Array1[3];

            textBoxLsVecp1.Refresh();
            textBoxLsVecp2.Refresh();
            textBoxLsVecp3.Refresh();
            textBoxLsVecp4.Refresh();
  ...
  #region  Disable Control
        ...
        private void button_Test_Click(object sender, EventArgs e)
        {
         ...
         //   DateTime StartTime = DateTime.Now.ToString();
            ...
            IsReadyOK = ReadyForTest();

            if (IsReadyOK)
            {
                pflowControl.MyLogManager.AdapterLogString(0, "***********************  SN=" + pflowControl.CurrentSN + " ************************");
            
                if (!pflowControl.MyDataio.WriterSN(pflowControl.StuctTestplanPrameter.Id, pflowControl.CurrentSN, pflowControl.Current_DutFwRev, pflowControl.StrIpAddress, StrLightSourceMessage, StrReMark, out pflowControl.SNID))
                {
                    pflowControl.MyLogManager.AdapterLogString(3, "WriterSN To Sql Error!");
                    IsReadyOK=false;
                    goto ErrorStop;
                }
#region  Local Xml Node
        ...
        private bool JudgeProductNo()
        {

            pflowControl.flag_SNOK = false;

          
               this.textBox_PN.Text= pflowControl.pDut.ReadPn().Trim();
               

               this.textBox_PN.Refresh();
        ...
        private void buttonOffset_Click(object sender, EventArgs e)
        {
            if (FlagEquipmentConfigOK)
            {
                try
                {
                    ...
                    //-----------------------------LightSourceVecp
                    LightSourceVecpArray.Clear();
                    LightSourceVecpArray.Add(textBoxLsVecp1.Text);
                    LightSourceVecpArray.Add(textBoxLsVecp2.Text);
                    LightSourceVecpArray.Add(textBoxLsVecp3.Text);
                    LightSourceVecpArray.Add(textBoxLsVecp4.Text);

                    string StrVecpArray = LightSourceVecpArray[0].ToString();

                    for (int i = 1; i < 4; i++)
                    {
                        StrVecpArray += "," + LightSourceVecpArray[i].ToString();
                    }
                    MyXml.LightSourceVecp = StrVecpArray;
                    pflowControl.StrLightSourceVecpArray = StrVecpArray;
...
#region   Config Equipment thread
        ...
        private void ConfigEquipment()
        {
            ...
            try
            {
                ...
                MyDutManufactureCoefficientsStructArray = pflowControl.GetDutManufactureCoefficients();//Map 系数
                MyManufactureChipsetStructArray = pflowControl.GetManufactureChipsetControl();//Driver Regist Addr
                MyDutManufactureChipSetInitialize = pflowControl.GetManufactureChipsetInitialize(); // Driver Initialize
                ...
                {
                    plogManager.AdapterLogString(1, "Get All Sql Paramter Fail!");
        ...
        {
            pflowControl.pDut.DutPRBS = 31;
          
            pflowControl.pDut.FullFunctionEnable();

            CFPExclusiveED pED = new CFPExclusiveED(new logManager());
            pED.MyParameter.Addr = 0;
            pED.MyParameter.IOType = "USB";
            pED.MyParameter.phycialAdress = 7;
            pED.MyParameter.GatingTime = 2;
            pED.MyParameter.PRBSLength = 31;
            pED.MyParameter.MoudleChannel = 1;
            pED.MyParameter.dDataRate = 25.78;
            pED.MyParameter.deviceIndex = 0;
            pED.Connect();
            double ErrorRate = pED.RapidErrorRate();
        ...
        {
         // bool k=  pflowControl.UpdateLocalXml();
        }
