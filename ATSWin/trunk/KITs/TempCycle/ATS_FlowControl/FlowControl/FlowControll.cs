    ...
    public class FlowControll// Flow Controll
    {
     ...
     //   public bool FlagDrvierNeedInitialize = false;
      ...
      //  public DataTable dtPnInf=new DataTable();
        ...
        public FlowControll(TempCycleXml MyXml, logManager alogManager)
        {
     ...
     Error:
         ...
         private bool SelcetTestModelParameter(string StrCurrentTestModelId)  
        {
            ...
            //// dtCurrentTestModeParameterlList.row

            DataRow[] drArray = dtCurrentConditionPrameter.Select("TestmodelId='" + StrCurrentTestModelId + "'");

            //dtCurrentTestModeParameterlList = dtCurrentConditionPrameter.Clone();

            //for (int i = 0; i < drArray.Length; i++)
            //{
            //    dtCurrentTestModeParameterlList.ImportRow(drArray[i]);
            //}
  ...
  static  public class TimeSpanUse
    {
      ...
      static public bool IsTimeInRang(TimeSpan tsBegin, int TimeRang)
      {
          int SleepTime = 0;
          TimeSpan tsEnd = new TimeSpan(DateTime.Now.Ticks);
          TimeSpan ts = tsEnd.Subtract(tsBegin).Duration();//时间差的绝对值 ，测试你的代码运行了多长时间。
           SleepTime = Convert.ToInt32(ts.TotalMilliseconds - TimeRang * 1000);
