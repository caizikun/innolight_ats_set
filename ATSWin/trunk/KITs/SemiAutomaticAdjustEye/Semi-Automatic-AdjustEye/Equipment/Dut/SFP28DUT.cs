                 ...
                 if (FindFiledNameChannelDAC(out i, StrItem))
                 {
                     ...
                     for (int J = 0; J < 3; J++)
                     {
                         ...
                         if (operate == 0)//写
                         {
                             ...
                             WriteDriver10g(deviceIndex, 0xA2, startaddr, DriverStruct[i].RegisterAddress, chipset, WriteDacByteArray);
                         }
                         else//存
                         {

                             StoreDriver10g(deviceIndex, 0xA2, startaddr, DriverStruct[i].RegisterAddress, chipset, WriteDacByteArray);

                         }

                         byte[] ReadDacByteArray = ReadDriver10g(deviceIndex, 0xA2, startaddr, DriverStruct[i].RegisterAddress, chipset);
