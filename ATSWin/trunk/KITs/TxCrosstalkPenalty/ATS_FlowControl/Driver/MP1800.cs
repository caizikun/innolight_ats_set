...
namespace ATS_Driver
{
    public class MP1800PPG : PPG
    {
         ...
         private bool ConfigureFromFile()
        {
         ...
         private bool ConfigureSlot(byte slot, int syn = 0)
         {
             ...
             try
             {
                 if (syn == 0)
                 {
                     logger.AdapterLogString(0, "MP1800ppg slot is" + slot);
                 ...
                 else
                 {
                     ...
                     if (flag1 == true)
                     {
                         ...
                         {
                             logger.AdapterLogString(3, "set MP1800ppg slot wrong");
                         }
                     }
         ...
         private bool ConfigureClockSource(byte clkSource, int syn = 0)//0=InternalClock
         {
            ...
            try
            {
                if (syn == 0)
                {
                    logger.AdapterLogString(0, "MP1800ppg ClockSource is" + strClkSource);
                ...
                else
                {
                    ...
                    if (flag1 == true)
                    {
                        ...
                        {
                            logger.AdapterLogString(3, "set MP1800ppg ClockSource wrong");
                        }
                    }
      ...
      public override bool RecallPrbsLength()
      {
        ...
        {
           return MyIO.WriteString(":SOURce:PATTern:DATA:LENGth " + length);
        }

        public override bool DataPattern_SetPatternData(string start, string end, string data)
        {
            return MyIO.WriteString(":SOURce:PATTern:DATA:WHOLe " + start + "," + end + ",\"" + data + "\"");
        }
        ...
        public override bool ConfigureOTxPolarity(bool polarity)
        {
          // :SOURce:PATTern:LOGic NEGative
            ...
            else
            {
             ...
             MyIO.WriteString(":SOURce:PATTern:LOGic " +StrPolarity);
             Thread.Sleep(200);
             MyIO.WriteString(":SOURce:PATTern:LOGic?");
        ...
        public override bool SetPPGDelay(double CurrentDelay, double OtherDelay)//设定PPG 的delay
        {
            try
            {
                ...
                MyIO.WriteString(":OUTPut:DATA:UIpadjust 0");
                GroupingSetting(false);

                ConfigureSlot(slot, 0);
                ConfigureChannel(Convert.ToByte( CurrentChannel));
               
                MyIO.WriteString(":OUTPut:DATA:UIpadjust " + CurrentDelay);

                return true;
            }
            catch
            {
                logger.AdapterLogString(3, "SetPPGDelay Error");
                return false;
            }
          
        }
        public override bool GroupingSetting(bool isSync)//是否要通道同步
        {
            try
            {
                ...
                if (isSync)
                {
                    //:SOURce:GROup:SETTing OUTPut,15
                    MyIO.WriteString(":SOURce:GROup:SETTing OUTPut,15");
                }
                else
                {
                    MyIO.WriteString(":SOURce:GROup:SETTing OUTPut,0");
                }
    ...
    public class MP1800ED : ErrorDetector
    {
        ...
        public override bool Initialize(TestModeEquipmentParameters[] MP1800EDStruct)
        {
            ...
            {
                logger.AdapterLogString(4, "there is no CURRENTCHANNEL");
                return false;
            }
            ...
            {
                logger.AdapterLogString(4, "there is no DATAINPUTINTERFACE");
                return false;
            }
            ...
            {
                logger.AdapterLogString(4, "there is no ERRORRESULTZOOM");
                return false;
            }
            ...
            {
                logger.AdapterLogString(4, "there is no EDGATINGMODE");
                return false;
            }
            ...
            {
                logger.AdapterLogString(4, "there is no EDGATINGUNIT");
                return false;
            }
        ...
        public override bool ChangeChannel(string channel, int syn = 0)
         ...
         public override bool AutoAlaign(bool becenter)
         {
             ...
             MyIO.WriteString(":SENSe:MEASure:AADJust32:STATe?");
             string Str = MyIO.ReadString();
             if (Str != "1\n")
             {
                 AutoAdjust();
         ...
         {
             //bool autoAjustResult = false;
             //MyIO.WriteString(":SYSTem:CFUNction AADJ32");  //open autosearch interface
             //MyIO.WriteString(":SENSe:MEASure:AADJust32:SLASet");//set all
             //Thread.Sleep(200);
             //MyIO.WriteString(":SENSe:MEASure:AADJust32:STARt");//start
             //MyIO.WriteString(":SYSTem:CFUNction OFF");  //close autosearch interface
             //return autoAjustResult;
        ...
        private bool ConfigureSlot(byte slot, int syn = 0)
        {
            ...
            try
            {
                if (syn == 0)
                {
                    logger.AdapterLogString(0, "MP1800 ED slot is" + slot);
                    return MyIO.WriteString(":MODule:ID " + slot.ToString() + "\n");
                }
                else
                {
                    ...
                    if (flag1 == true)
                    {
                        ...
                        {
                            logger.AdapterLogString(3, "MP1800 ED set slot wrong");
                        }
                    }
        ...
        private bool ConfigureChannel(int channel, int syn = 0)
        {
            ...
            try
            {
                if (syn == 0)
                {
                    logger.AdapterLogString(0, "MP1800 ED channel is" + channel);
                    return MyIO.WriteString(":INTerface:ID " + channel.ToString());      
                ...
                else
                {
                    ...
                    if (flag1 == true)
                    {
                        ...
                        {
                            logger.AdapterLogString(3, "set MP1800 ED channel wrong");
            ...
            try
            {
                for (int i = 0; i < 3; i++)
                {
                    try
                    {
                        ...
                        Thread.Sleep(100);
                        StrBer = MyIO.ReadString(100).Replace("\"", "").Replace("\n", "");
                        Thread.Sleep(100);
                    ...
                    catch (Exception ex)
                    {
                        ...
                        logger.AdapterLogString(0, ex.Message);
                    }
        ...
        public override bool ConfigureERxPolarity(bool polarity)
        {
            ...
            MyIO.WriteString(":SENSe:PATTern:LOGic " + StrPolarity);
            Thread.Sleep(200);
            MyIO.WriteString(":SENSe:PATTern:LOGic?");
            string Str = MyIO.ReadString();
            if (Str.Contains(StrPolarity))
            {
                return true;
