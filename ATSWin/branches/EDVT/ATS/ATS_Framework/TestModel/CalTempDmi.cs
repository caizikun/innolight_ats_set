...
{
   public class CalTempDmi : TestModelBase
    {
#region Attribute
...
#endregion
#region  Method
        public CalTempDmi(DUT inPuDut, logManager logmanager)
        {
            logger = logmanager;
            logoStr = null;
            dut = inPuDut;         
                ...
                if (tempCount >= 2)
                {
                    ...
                    else if (calTempDmiStruct.is1Stor2StorPid == 1)
                    {
                        ...
                        for (byte i = 0; i < GlobalParameters.TotalTempCount; i++)
                        {
                            ...
                            tempDmiCoefC = (float)coefArray[0];
                            tempDmiCoefB = (float)coefArray[1];
                            tempDmiCoefC = Math.Abs(tempDmiCoefC) / 100;
                            tempDmiCoefB = Math.Abs(tempDmiCoefB) * 100;
