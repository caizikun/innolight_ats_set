...
namespace ATS_Framework
{
 ...
 #endregion
 #region Method
        public TestIcc(DUT inPuDut, logManager logmanager)
        {
            logger = logmanager;
            dut = inPuDut;
            logoStr = null;            
        ...
        protected override bool StartTest()
        {
            ...
            if (selectedEquipList["DUT"] != null && selectedEquipList["POWERSUPPLY"] != null)
            {
                ...
                // open apc
                logoStr += logger.AdapterLogString(0, "Step3..Read ICC");
                icc = tempps.GetCurrent();
                icc -= GlobalParameters.Iccoffset;
                logoStr += logger.AdapterLogString(1, "ICC=" + icc.ToString());
        ...
        protected override bool AnalysisOutputParameters(TestModeEquipmentParameters[] InformationList)
        {
            ...
            else//  InformationList is not null
            {
                ...
                for (byte i = 0; i < InformationList.Length; i++)
                {
                    ...
                    if (algorithm.FindFileName(InformationList, "ICC(MA)", out index))
                    {
                        InformationList[index].DefaultValue = Math.Round(icc,4).ToString().Trim();
                    }
                }
