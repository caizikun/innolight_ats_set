...
{
   public class CalVccDminoProcessingCoef : TestModelBase
    {
#region Attribute
...
#endregion
#region Method
        public CalVccDminoProcessingCoef(DUT inPuDut, logManager logmanager)
        {
            calledCount = 0;
            logger = logmanager;
            logoStr = null;
            ...
            if (selectedEquipList["POWERSUPPLY"] != null && selectedEquipList["DUT"] != null)
            {
                ...
                if (calVccDmiStruct.is1Stor2StorPid==1)
                {
                    ...
                    for (byte i = 0; i < GlobalParameters.TotalVccCount;i++)
                    {
                        ...
                        for (byte k = 0; k < vccDmiCoefArray.Count; i++)
                        {
            ...
            else
            {
                ...
                if (isParametersComplete)
                {
                    for (byte i = 0; i < InformationList.Length; i++)
                    {
                        ...
                        if (algorithm.FindFileName(InformationList, "VCCOFFSET(V)", out index))
                        {
                            GlobalParameters.VccOffset = Convert.ToDouble(InformationList[index].DefaultValue);
