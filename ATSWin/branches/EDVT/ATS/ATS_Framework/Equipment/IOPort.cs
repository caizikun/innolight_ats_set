...
namespace ATS_Framework
{
    ...
    public class IOPort
    {
        ...
        public void FpgaInrush(int deviceIndex, bool regAddressWide, int command, byte operate)
           
        {
            for (int i = 0; i < 10; i++)
            {
                OpenDevice(deviceIndex);
                if(ReadID(deviceIndex)==1429685064)
                    break;
        ...
        public byte[] FpgaIIC(int deviceIndex , int regAddress,bool regAddressWide, int command, byte operate,byte[] buffer)
        {
            for (int i = 0; i < 10; i++)
            {
                OpenDevice(deviceIndex);
            ...
            byte[] arr = new byte[buffer.Length + 6];

            arr[0] = regAddressWide ? (byte)1 : (byte)0;
            arr[1] = (byte)operate;
            arr[2] = (byte)command;
            arr[3] = (byte)(regAddress / 256);
            arr[4] = (byte)(regAddress & 0xFF);
            arr[5] = (byte)buffer.Length;
            buffer.CopyTo(arr, 6);
            bool b = CH375WriteData(deviceIndex, arr);
            byte[] arrRead = CH375ReadData(deviceIndex, buffer.Length);
            CloseDevice(deviceIndex);
        ...
        public byte[] FpgaIICRead(int deviceIndex, int regAddress, int command, int readLength)
        {
            return FpgaIIC(deviceIndex, regAddress, false, command, 1,new byte[readLength]);
        }

        public byte[] FpgaIICWrtie(int deviceIndex, int regAddress, int command,byte[] dataToWrite)
        {
            return FpgaIIC(deviceIndex, regAddress, false, command, 0,dataToWrite);
        }
        public void FpgaResetTimeSet(int deviceIndex, bool regAddressWide, int command,double delaytime)
        {
            for (int i = 0; i < 10; i++)
            {
                OpenDevice(deviceIndex);
            ...
            arr[0] = regAddressWide ? (byte)1 : (byte)0;
            arr[1] = regAddressWide ? (byte)1 : (byte)0;
            arr[2] = (byte)command;
            arr[3] = (byte)(delaytime*10);
            bool b = CH375WriteData(deviceIndex, arr);
            CloseDevice(deviceIndex);
            System.Threading.Thread.Sleep(200);
        ...
        public void McuFpgaModeSel(int deviceIndex, bool regAddressWide, int command, byte operate,byte ICSelect)
        {
            for (int i = 0; i < 10; i++)
            {
                OpenDevice(deviceIndex);
        ...
        public void FpgaNorsineTxRx(int deviceIndex, bool regAddressWide, int command, byte operate, int acamplitude,double acfreq,double dcamplitude)
        {
            byte[] acamptemp=new byte[2];
            byte[] acfreqtemp = new byte[4];
            byte[] dcamptemp = new byte[4];
            for (int i = 0; i < 10; i++)
            {
                OpenDevice(deviceIndex);
                if (ReadID(deviceIndex) == 1429685064)
            ...
            acamptemp[0] = (byte)(acamplitude / 256);
            acamptemp[1] = (byte)(acamplitude & 0xFF);
            acfreqtemp = BitConverter.GetBytes(acfreq);
            dcamptemp = BitConverter.GetBytes(dcamplitude);
        ...
        public byte[] SamplePowerCurrent(int deviceIndex, bool regAddressWide, byte datalength)
        {
            for (int i = 0; i < 10; i++)
            {
                OpenDevice(deviceIndex);
        ...
        public void FpgaSwitch(int deviceIndex, bool regAddressWide, int command, byte operateh,byte operatel)
        {
            for (int i = 0; i < 10; i++)
            {
                OpenDevice(deviceIndex);
