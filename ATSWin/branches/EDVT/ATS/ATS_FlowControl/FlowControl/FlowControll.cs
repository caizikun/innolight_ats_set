...
namespace ATS
{
    ...
    public class FlowControll// Flow Controll
    {
      ...
      //  private WriteTxT pWriteTxt;
        ...
        private globalParameters pGlobalParameters = new globalParameters();
        ...
        //public bool GetProductionInf()
        //{

        //    ReadProductionTpye();
        //    GetIdFromTpyeTable("QSFP", out ProductionTypeId, out MSAID);
        //    return true;

        //}
        private delegate void MyProgressDelegate(int k);
        private event MyProgressDelegate MyEvent;
        ...
        public CurrentTestModelEquipmentList BuildCurrentTestModelEquipmentList(string[] ArrayEquipmenName)
        {

            CurrentTestModelEquipmentList pCurrentTestModelEquipmentList = new CurrentTestModelEquipmentList();

            if (ArrayEquipmenName != null)
            {

                foreach (string Str in ArrayEquipmenName)
                {
                    if (!MyEquipList.Keys.Contains(Str.ToUpper()))
                    {
                        MessageBox.Show(Str.ToUpper() + "不存在");
                    }
                    else
                    {
                        pCurrentTestModelEquipmentList.Add(Str.ToUpper(), MyEquipList[Str.ToUpper()]);
                    }
                }
            }
            return pCurrentTestModelEquipmentList;
        }
        ...
        public bool RunTestModel(string StrTestModelName, String StrCurrentTestModelId)
        {
            ...
            try
            {
                ...
                //-------------------------------- 组件TestModel 所需仪器列表
                if (drTestModel["EquipmentList"].ToString() != null && drTestModel["EquipmentList"].ToString() != "")
                {
                    ArrayEquipmenName = drTestModel["EquipmentList"].ToString().Split(',');//获取当前Testmodel 所需要的仪器

                }
                else
                {
                    ArrayEquipmenName = null;
                }

                MyTestModelList[StrTestModelName.ToUpper().Trim()].SelectEquipment(BuildCurrentTestModelEquipmentList(ArrayEquipmenName));// 将其要使用的仪器 加入当前的Testmodel 自己的 sortlist
               ...
               // MyLogManager.AdapterLogString(1,)
                ...
                string StrSN = SerialNO + "\r\n"; ;
                //StrTotalLog += StrSN + TestStartTime + ”+"\r\n";;
                //   StrTotalLog += MyTestModelList[StrTestModelName.ToUpper().Trim()].GetLogInfor;
                //StrTotalLog += TestEndTime;
                ...
                MyJudgeSpec.Judge(dtCurrentTestModelTestData, TestModelOutputArray,CurrentTestModelflag, out dtCurrentTestModelTestData, out StopFlag,out CurrentTestModelResultflag);

                if (!CurrentTestModelResultflag)  TotalResultFlag  = false;
        ...
        public bool SelcetTestModelParameter(string StrCurrentTestModelId)
        {
            ...
            foreach (DataRow drTestModelParameter in dtCurrentTestModeParameterlList.Rows)// 遍历当前Testmodel 所需要的所有参数
            {
                ...
                if (drTestModelParameter["Direction"].ToString().ToUpper().Trim() == "OUTPUT" || drTestModelParameter["ItemSpecific"].ToString().ToUpper().Trim() == "1")
                {
                    ...
                    dr["DefaultLowLimit"] = drTestModelParameter["DefaultLowLimit"];
                    dr["DefaultUpperLimit"] = drTestModelParameter["DefaultUpperLimit"];
        ...
        public SortedList<string, string> GetEquipmentNameList(int idTestplan)
        {
            int i = 0;

            SortedList<string, string> pEquipment = new SortedList<string, string>();

          
               
            string StrTableName = "TopoEquipment";

            string Selectconditions = "select * from " + StrTableName + " where PID=" + idTestplan + " order by ID";
            ...
            for (i = 0; i < DT.Rows.Count; i++)
            {
                ...
                pEquipment.Add(DT.Rows[i]["ItemName"].ToString().ToUpper(), DT.Rows[i]["ID"].ToString().ToUpper());
