...
namespace ATS
{
     ...
     */
      ...
      //  private string[,] EquipmenNameArray;
        ...
        private string PrtScPath = @"D:\PrtSc";
...
#region  委托声明
          ...
          //  AnimateWindow(this.Handle, 1000, AW_CENTER);
            ...
            if (MyXml.DataBaseUserLever == "1")
            {
                ...
                if (MessageBox.Show("Do you want to Use Location Database?", "Database Message", MessageBoxButtons.OKCancel, MessageBoxIcon.Question) == DialogResult.OK)
                {
                    ...
                    pflowControl = new FlowControll(MyXml);
                }
                else
                {
        ...
        private bool ReadyForTest()
        {
           ...
           // Powersupply aPS;
            ...
            Log.SaveLogToTxt("$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$A New Test Start$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$");
            Log.SaveLogToTxt("chK_Fw Funtion=" + chK_Fw.CheckState);
            Log.SaveLogToTxt("chk_AbNormal_Lab Funtion=" + chk_AbNormal_Lab.CheckState);
            Log.SaveLogToTxt("chK_Aotu_Evb_Voltage Funtion=" + chK_Auto_Evb_Voltage.CheckState);
  ...
  #region  Disable Control
                ...
                try
                {
                   ...
                   pflowControl.pDut.FullFunctionEnable();
                    ...
                    TimeSpanUse.DateDiff(pflowControl.TimeSpanStart, "Prepare Time for Test -JudgeSerialNO ", out pflowControl.TimeSpanStart);

                    if (!pflowControl.pDut.FullFunctionEnable()) return false;

                   if(!JudegEvb())return false;
                   TimeSpanUse.DateDiff(pflowControl.TimeSpanStart, "Prepare Time for Test -JudegEvb ", out pflowControl.TimeSpanStart);
                   ...
                   TimeSpanUse.DateDiff(pflowControl.TimeSpanStart,"Prepare Time for Test -JudgeFwVer ", out pflowControl.TimeSpanStart);
        ...
        private void button_Test_Click(object sender, EventArgs e)
        {
         ...
         //   DateTime StartTime = DateTime.Now.ToString();
            ...
            if (pflowControl.pThermocontroller != null)
            {
                pflowControl.pThermocontroller.SetPointTemp(pflowControl.StartTemp, 1);
                pflowControl.pThermocontroller.SetPositionUPDown("1");
            }

            if (ReadyForTest())
            {
                string fileName = pflowControl.CurrentSN + "_" + DateTime.Now.ToString("yyyyMMddHHmmss") + ".txt";
                FilePath.LogFile = Application.StartupPath + @"\Log\" + fileName;
                //pflowControl.MyErrorLogManager = new logManager(pflowControl.StrPathErrorLog, pflowControl.CurrentSN);
                Log.SaveLogToTxt("A New Dut Test Start--------------------");
                //pflowControl.MyErrorLogManager.AdapterLogString("KK");
               // pflowControl.MyErrorLogManager.FlushLogBuffer();

                Log.SaveLogToTxt("***********************  SN=" + pflowControl.CurrentSN + " ************************");
                ...
                string StrTime = Str_Year + "_" + Str_Month + "_" + Str_Day + "_" + Str_Hour + "_" + Str_minute + "_" + Str_sencond;


                pflowControl.StrPathLocalXml = pflowControl.StrPathLocalDirectory + "\\" + pflowControl.CurrentSN + "-" + StrTime + ".xml";

                pflowControl.MyLocatDataXml = new LocatDataXml(pflowControl.StrPathLocalXml, new RunRecordTableColums(pflowControl.CurrentSN, pflowControl.StuctTestplanPrameter.Id.ToString(), DateTime.Now.ToString(), DateTime.Now.ToString(), pflowControl.Current_DutFwRev, pflowControl.StrIpAddress, StrLightSourceMessage, StrReMark));

                // pflowControl.MyLocatDataXml.CreartSnNode(new RunRecordTableColums(pflowControl.CurrentSN, pflowControl.StuctTestplanPrameter.Id.ToString(), DateTime.Now.ToString(), DateTime.Now.ToString(), pflowControl.Current_DutFwRev, pflowControl.StrIpAddress, StrLightSourceMessage, StrReMark));
        ...
        private bool JudegEvb()
        {
            ...
            if (pflowControl.Flag_NeedAutoCheck_EvBVoltage)
            {
               // Flag_Auto_Check_EvBVoltage
                ...
                Log.SaveLogToTxt("测试板FlagCal=" + flag.ToString());
        ...
        private void Test()
        {
            if (this.checkBoxParallel.Checked)
            {
                if (!pflowControl.ParallelStartTest())
                {
                ...
                if (TestThread != null)
                {
                    if (!pflowControl.FlagFlowControllEnd)
                    {
                       // MessageBox.Show("测试还没结束，关你妹啊！");
                        ...
                        Log.SaveLogToTxt("You Have Illicit close the ATS EXE");
                    }
                    TestThread.Abort();
                }
        ...
        private bool GetTestplanInf()
        {
            ...
            TestplanPrameter.DutUsbPort = Convert.ToByte(dt.Rows[0]["USBPort"]);
            pflowControl.StuctTestplanPrameter.IsCDRon = Convert.ToBoolean(dt.Rows[0]["IsCDROn"]);
        ...
        {
            FilePath.LogFile = Application.StartupPath + @"\Log\" + "Config_" + DateTime.Now.ToString("yyyyMMddHHmmss") + ".txt";
            ...
            Log.SaveLogToTxt("Config Equipment-------------------");
...
#region 创建存放眼图的文件夹位置
            ...
            string StrPathOptEyeDiagram = PrtScPath + @"\EyeDiagram\" + comboBoxPType.Text.ToUpper() + "\\" + comboBoxPN.Text.ToUpper() + "\\" + comboBoxTestPlan.Text.ToUpper() + "\\" + StrTime + "\\OptEyeDiagram\\";
            string StrPathElecEyeDiagram = PrtScPath + @"\EyeDiagram\" + comboBoxPType.Text.ToUpper() + "\\" + comboBoxPN.Text.ToUpper() + "\\" + comboBoxTestPlan.Text.ToUpper() + "\\" + StrTime + "\\ElecEyeDiagram\\";
            ...
            string StrPathErrorLog = PrtScPath + @"\ErrorLog\";
           ...
           // pflowControl.StrEyeDiagramPath = StrPathEyeDiagram;
            ...
            if (!Directory.Exists(StrPathPolarityEyeDiagram))
            {
                Directory.CreateDirectory(StrPathPolarityEyeDiagram);
            }
            if (!Directory.Exists(StrPathErrorLog))
            {
                Directory.CreateDirectory(StrPathErrorLog);
            }
            // pflowControl.StrEyeDiagramPath = StrPathEyeDiagram;
            pflowControl.StrPathPolarityEyeDiagram = StrPathPolarityEyeDiagram + "\\";
            pflowControl.StrPathErrorLog = StrPathErrorLog;
            ...
            pflowControl.StrPathBackUpirectory = PrtScPath + @"\BackUp";
            pflowControl.StrPathLocalDirectory = Application.StartupPath + @"\CatchData";

            if (!Directory.Exists(pflowControl.StrPathLocalDirectory))
            {
                Directory.CreateDirectory(pflowControl.StrPathLocalDirectory);
            }
            if (!Directory.Exists(pflowControl.StrPathBackUpirectory))
            {
                Directory.CreateDirectory(pflowControl.StrPathBackUpirectory);
            }
            #endregion
...
#region Disable Control
            ...
            try
            {
                if (comboBoxTestPlan.Text != "")
                {
                    ...
                    if (this.checkBoxParallel.Checked)
                    {
                        Config = new Thread(ParallelConfigEquipment);
                    }
                    else
                    {
                        Config = new Thread(ConfigEquipment);
                    }
                    Config.Start();
...
#region   Config Equipment thread
        ...
        private void ConfigEquipment()
        {
            ...
            try
            {
                ...
                DUT.deviceIndex = TestplanPrameter.DutUsbPort;
                pflowControl.pDut.IsCDROn = pflowControl.StuctTestplanPrameter.IsCDRon;
                ...
                for (i = 0; i < dtEquipmenList.Rows.Count; i++)
                {
                   ...
                   // string[] StrArray = EquipmenNameArray[i, 0].Split('_');
                    ...
                    if (CurrentEquipmentFullName.Contains("POWERSUPPLY"))
                    {
                   ...
                   // pThermocontroller
                    ...
                    Log.SaveLogToTxt(CurrentEquipmentName + "  ConfigOK");
                }

                if (!pflowControl.GetAllConditionList() || !pflowControl.GetAllTestModelList() || !pflowControl.GetAllTestModelParamterList())
                {
                    Log.SaveLogToTxt("Get All Sql Paramter Fail!");
                    FlagEquipmentConfigOK = false;//获取所有Sql测试需要的参数TestmodelParamter;
            ...
            {
               // plogManager.AdapterLogString(3, "ErrorCode=" + ExceptionDictionary.Code._0x05002 + "Reason=" + error.TargetSite.Name + "Fail");
                Log.SaveLogToTxt(CurrentEquipmentName + "  Config Error");

                MessageBox.Show(CurrentEquipmentName+error.Message, "ConfigEquipment Error ,PLS Check Equipment");
                UpDataStatus(this.labelShow, CurrentEquipmentName+" Config false");
                FlagEquipmentConfigOK = false;
            }
            catch (Exception EX)
            {
              ...
              // UpDataStatus(this.labelShow, CurrentEquipmentName+"Equipment Config false");
               UpDataStatus(this.labelShow, CurrentEquipmentName + " Config false");
                FlagEquipmentConfigOK = false;
            }
            finally
            {
                //pflowControl.MyLogManager.FlushLogBuffer();
                PanelUpdata(this.panelCombox, true);
                PanelUpdata(this.panelOffset, true);
        ...
        private void ParallelConfigEquipment()
        {
            ...
            try
            {
                ...
                int j = 0;
                Parallel.For(0, dtEquipmenList.Rows.Count, (int i, ParallelLoopState pls) =>
                {
                    if (FlagEquipmentConfigOK == false)
                    {
                        pls.Break();
                        return;
                    }
                    TestModeEquipmentParameters[] CurrentEquipmentStruct = pflowControl.GetCurrentEquipmentInf(dtEquipmenList.Rows[i]["id"].ToString());
                    // string[] StrArray = EquipmenNameArray[i, 0].Split('_');
                    string CurrentEquipmentName = dtEquipmenList.Rows[i]["Name"].ToString().ToUpper();
                    string CurrentEquipmentFullName = dtEquipmenList.Rows[i]["FullName"].ToString();
                    EquipmentBase CurrentEquipmentObject = (EquipmentBase)MyEquipmentManage.Createobject(CurrentEquipmentName);
                    CurrentEquipmentObject.Role = Convert.ToByte(dtEquipmenList.Rows[i]["Role"]);// 0=NA,1=TX,2=RX

                    UpDataStatus(labelShow, CurrentEquipmentName + "  Configing.....");
                    Log.SaveLogToTxt(CurrentEquipmentName + "  Configing.....");
                    if (!CurrentEquipmentObject.Initialize(CurrentEquipmentStruct) || !CurrentEquipmentObject.Configure(1))
                    {
                        MessageBox.Show(CurrentEquipmentName + "Configure Error");
                        FlagEquipmentConfigOK = false;
                    ...
                    CurrentEquipmentObject.OutPutSwitch(false);
                    j++;
                    int ProcessValue = (int)j * 100 / (dtEquipmenList.Rows.Count);
                    UpDataProcess(progress, ProcessValue);
                    UpDataStatus(labelProgress, ProcessValue.ToString());
                    UpDataStatus(labelShow, CurrentEquipmentName + "  ConfigOK");
                    Log.SaveLogToTxt(CurrentEquipmentName + "  ConfigOK");
            ...
            {
                // plogManager.AdapterLogString(3, "ErrorCode=" + ExceptionDictionary.Code._0x05002 + "Reason=" + error.TargetSite.Name + "Fail");
                Log.SaveLogToTxt("Config Error");

                MessageBox.Show(error.Message, "ConfigEquipment Error ,PLS Check Equipment");
                UpDataStatus(this.labelShow, " Config false");
                FlagEquipmentConfigOK = false;
            }
            catch (Exception EX)
            {
        ...
        private void buttonStop_Click(object sender, EventArgs e)    
        {
            ...
            if (MessageBox.Show("Do you want to Stop Test?", " Message", MessageBoxButtons.OKCancel, MessageBoxIcon.Question) == DialogResult.OK)
            {
                ...
                Log.SaveLogToTxt("You Have Stop the ATS EXE");
                buttonStop.Enabled = false;   
            }
        ...
        private void comboBoxTestPlan_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (comboBoxPN.Text != "")
            {
                ...
                pflowControl.StuctTestplanPrameter.TestplanDescription=dt.Rows[0]["ItemDescription"].ToString();
                ...
                Text_Remark.Visible = true;
                Text_Remark.Enabled = false;
                Text_Remark.Text = pflowControl.StuctTestplanPrameter.TestplanDescription;
                Text_Remark.Refresh();
        ...
        private void button1_Click_1(object sender, EventArgs e)
        {
            ...
            int JoinValue = Algorithm.WriteJointBitValue(3800, 3888, 2, 0, 11);//写入值处理
            pflowControl.UpdateLocalXml();
          //  string S = pflowControl.pDut.ReadSn();
        }
